...
void CursorView(bool show) // 출력 콘솔의 커서 깜빡임 제거
{
    HANDLE hConsole;
...
void print(string square[][num], string stone, int areaX, int areaY) // 바둑판을 출력해주는 함수
{
    system("cls"); // 지금까지 콘솔에  출력한 화면을 클리어 해줌
    for (int i = 0; i < num; i++)
    {
        for (int j = 0; j < num; j++)
        {
    ...
    cout << "           " << setw(5) << "A←" << setw(4) << "+" << setw(6) << "→D"
         << "     SPACE BAR : 바둑돌 두기 " << endl;
...
int main()
{
    ...
    { // 게임 오프닝
        ...
        cout << "*************************************************************" << endl;
        moove = _getch(); // 입력값을 버퍼에 저장하지 않고 입력된 값을 바로바로 반환해주는 함수
        if (moove == 13)
        { // 엔터 입력으로 게임 시작
            break;
        }
