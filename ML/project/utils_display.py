    ...
    def draw2d(self, img, param):
        ...
        for p in param:
            if p['class'] is not None:
                ...
                hand_label = 'Right' if p['class'] == 'Left' else 'Left'
                
                # Label left or right hand
                x = int(p['keypt'][0,0]) - 30
                y = int(p['keypt'][0,1]) + 40
                cv2.putText(img, '%s' % (hand_label), (x, y), 
                ...
                for i in range(21):
                    ...
                    if p['gesture'] is not None:
                        gesture_text = str(p['gesture']).upper()  # 정수를 문자열로 변환
                        size = cv2.getTextSize(gesture_text, cv2.FONT_HERSHEY_SIMPLEX, 1, 2)[0]
                        x1 = int(p['keypt'][0,0]) - 30
                        y1 = int(p['keypt'][0,1]) + 80
                        cv2.putText(img, gesture_text,
                                    (x1, y1), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
