...
class HandRomRecognition:
    ...
    def eval(self, angle):
        ...
        data = np.asarray([angle], dtype=np.float32)
        ret, results, neighbours, dist = self.knn.findNearest(data, 3)
        if results.size > 0 and int(results[0][0]) in self.gesture:
            idx = int(results[0][0])  # Index of class label
            return self.gesture[idx]  # Return name of class label
        else:
...
class WristArmRom:
    ...
    def eval(self, param):
        ...
        if self.mode == 0 or self.mode == 1:
            ...
            if self.side == 'left':
                # Note: Temporary make use hand joint angles
                param_lh['angle'][0] = angle
        ...
        # Forearm pronation/supination
            ...
            # Vector pointing from wrist to index finger MCP
            v1 = param[0]['joint'][5] - param[0]['joint'][0]
            # Vector pointing from wrist to little finger MCP
            v2 = param[0]['joint'][17] - param[0]['joint'][0]
